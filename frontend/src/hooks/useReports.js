...
export const useGeneratePDF = () => {
  ...
  return useMutation({
    ...
    onSuccess: (response, variables) => {
      // PDF generation is now synchronous, so we can update with the final result
      const updatedReport = response.data.report;
      
      // Update the specific report in cache with the completed data
      queryClient.setQueryData(QUERY_KEYS.reports, (old = []) => {
        return old.map(report => 
          report.id === variables.reportId ? updatedReport : report
        );
      });
